 ...
 * Requires cpLoss from engine (played line vs best line).
 */
export function classifyMoveGrade(input: {
  ...
  const { cpLoss, playedBest, ply, userEvalBeforeCp } = input;
  ...
  if (cpLoss <= CP_INACCURACY) return "good";
...
export function insightForGrade(
  ...
  switch (grade) {
    case "brilliant":
      return "Brilliant — the engine's top choice in a critical position.";
    case "best":
